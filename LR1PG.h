...
namespace LR1PG
{
	...
	public:
		...
		action(const string &str)
		{
			if(str.compare("acc")==0)
			{
				this->type=action_type::accept ;
				return;
			}
			if(str.compare("err")==0)
			{
				this->type=action_type::error ;
				return;
			}
			if(str[0]=='s')
			{
				this->type=action_type::shift ;
				string ss=str.substr(1);
				this->index=atoi(ss.c_str());
			}
			else
			{
				this->type=action_type::reduction ;
				string ss=str.substr(1);
				this->index=atoi(ss.c_str());
			}
