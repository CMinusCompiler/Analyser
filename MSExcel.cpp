...
int CMSExcel::Initialize(bool bVisible)
{
	CoInitialize(NULL);
	CLSID clsid;
...
int CMSExcel::SetVisible(bool bVisible)
{
/*	DISPID dispID;
	VARIANT pvResult;
...
int CMSExcel::OpenExcelBook(LPCTSTR szFilename, bool bVisible)
{
	if(m_pEApp==NULL) 
	{
...
int CMSExcel::NewExcelBook(bool bVisible)
{
	if(m_pEApp==NULL) 
	{
		if(FAILED(m_hr=Initialize(bVisible)))
...
int CMSExcel::SaveAs(LPCTSTR szFilename, int nSaveAs)
{
...
int CMSExcel::ProtectExcelWorkbook(LPCTSTR szPassword)
{
	if(!m_pEApp) return E_FAIL;
...
int CMSExcel::UnProtectExcelWorkbook(LPCTSTR szPassword)
{
	if(!m_pEApp) return E_FAIL;
	if(!m_pActiveBook) return E_FAIL;
	COleVariant olePassword(szPassword);
...
int CMSExcel::ProtectExcelSheet(int nSheetNo, LPCTSTR szPassword)
{
	if(!m_pEApp) return E_FAIL;
	if(!m_pActiveBook) return E_FAIL;
	IDispatch *pSheets;
...
int CMSExcel::UnProtectExcelSheet(int nSheetNo, LPCTSTR szPassword)
{
	if(!m_pEApp) return E_FAIL;
	if(!m_pActiveBook) return E_FAIL;
	IDispatch *pSheets;
...
int CMSExcel::SetExcelCellFormat(LPCTSTR szRange, LPCTSTR szFormat)
{
	if(!m_pEApp) return E_FAIL;
	if(!m_pActiveBook) return E_FAIL;
	IDispatch *pSheets;
...
int CMSExcel::SetExcelSheetName(int nSheetNo, LPCTSTR szSheetName)
{
	if(!m_pEApp) return E_FAIL;
	if(!m_pActiveBook) return E_FAIL;
	IDispatch *pSheets;
...
int CMSExcel::GetExcelValue(LPCTSTR szCell, CString &sValue)
{
	if(!m_pEApp) return E_FAIL;
	if(!m_pActiveBook) return E_FAIL;
...
int CMSExcel::SetExcelBackgroundColor(LPCTSTR szRange, COLORREF crColor, int nPattern)
{
	if(!m_pEApp) return E_FAIL;
	if(!m_pActiveBook) return E_FAIL;
	IDispatch *pSheets;
...
int CMSExcel::SetExcelFont(LPCTSTR szRange, LPCTSTR szName, int nSize, COLORREF crColor, bool bBold, bool bItalic)
{
	if(!m_pEApp) return E_FAIL;
	if(!m_pActiveBook) return E_FAIL;
	IDispatch *pSheets;
...
int CMSExcel::SetExcelValue(LPCTSTR szRange,LPCTSTR szValue,bool bAutoFit, int nAlignment)
{
	if(!m_pEApp) return E_FAIL;
	if(!m_pActiveBook) return E_FAIL;
	IDispatch *pSheets;
...
int CMSExcel::SetExcelBorder(LPCTSTR szRange, int nStyle)
{
	if(!m_pEApp) return E_FAIL;
	if(!m_pActiveBook) return E_FAIL;
	IDispatch *pSheets;
...
int CMSExcel::MergeExcelCells(LPCTSTR szRange)
{
	if(!m_pEApp) return E_FAIL;
	if(!m_pActiveBook) return E_FAIL;
	IDispatch *pSheets;
...
int CMSExcel::AutoFitExcelColumn(LPCTSTR szColumn)
{
	if(!m_pEApp) return E_FAIL;
	if(!m_pActiveBook) return E_FAIL;
	IDispatch *pSheets;
...
int CMSExcel::AddExcelChart(LPCTSTR szRange, LPCTSTR szTitle, int nChartType, int nLeft, int nTop, int nWidth, int nHeight, int nRangeSheet, int nChartSheet)
{
	if(!m_pEApp) return E_FAIL;
	if(!m_pActiveBook) return E_FAIL;
	IDispatch *pSheets;
...
int CMSExcel::MoveCursor(int nDirection)
{
...
int CMSExcel::GetActiveCell(int &nRow, int &nCol)
{
	if(!m_pEApp || !m_pActiveBook) return E_FAIL;
